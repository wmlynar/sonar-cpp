...
public class CxxFunction extends Resource<CxxFile>{
...
//	public static final String SCOPE = Scopes.BLOCK_UNIT;
	...
	{
		 logger.debug("initIdentifierFromAbsolutFilePath received : {}", absoluteFilePath);
		 file = CxxFile.fromFileName(project, absoluteFilePath, false);
/*		String filename = StringUtils.substringAfterLast(absoluteFilePath, "/");
		 
		file = CxxFile.fromFileName(project, filename, false);*/
		String key =  file.getParent().getProjectPath() + ":" + file.getName() + ":" + functionName;
		logger.debug("key =  {}", key.replace('/', ':'));
		setKey(key.replace('/', ':'));
	}
	
	private CxxFunction(Project project, String fileName, String functionName, boolean unitTest) {
		if (fileName == null) {
		...
		qualifier = Resource.QUALIFIER_METHOD;// unitTest ? Resource.QUALIFIER_UNIT_TEST_CLASS : Resource.QUALIFIER_METHOD;//Resource.QUALIFIER_FILE;
...
/*			for (String includePath : includeSearchPath) {
	...
	public static CxxFunction fromFileName(Project project, String filename, String name, boolean unitTest) {
		logger.debug("CxxFunction fromFileName:  filename = " + filename + " function = " + name);
		return new CxxFunction(project, filename, name, unitTest);
	}
}
