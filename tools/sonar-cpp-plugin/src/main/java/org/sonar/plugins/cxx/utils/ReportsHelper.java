...
public abstract class ReportsHelper {
	...
	public File getReportsDirectory(Project project) {
		...
		// if it is module check parent directory
		if (report == null) {
			report = getReportDirectoryFromParent(project);
			getLogger().info("getReportDirectoryFromParent() = " + report);
		}
		if (report == null) {
	...
	private File getReportDirectoryFromParent(Project project) {
		if (project.isModule()){
			return project.getParent().getPom().getBasedir();
		}
		return null;
	}
