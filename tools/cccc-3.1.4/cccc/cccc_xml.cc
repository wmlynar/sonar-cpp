...
static const string JAVADOC_NODE_NAME       = "lines_of_javadoc"; /* WOJ */
static const string VISIBILITY_NODE_NAME       = "visibility"; /* WOJ */
static const string NOCCCC_NODE_NAME       = "nocccc"; /* WOJ */
static const string MAXNESTING_NODE_NAME       = "max_nesting"; /* WOJ */
static const string LINES_NODE_NAME       = "lines"; /* marcin */
  ...
  while(mod_ptr!=NULL)
      ...
      if( mod_ptr->is_trivial() == FALSE)
	{
	  ...
	  Put_Metric_Node(MVGPERCOM_NODE_NAME,mm_c);
          ...
          /* woj JAVADOC */
	  int javadoc=mod_ptr->get_count("JAVADOC");
	  Put_Metric_Node(JAVADOC_NODE_NAME,javadoc);

          /* woj NOCCCC */
	  int nocccc=mod_ptr->get_count("NOCCCC");
	  Put_Metric_Node(NOCCCC_NODE_NAME,nocccc);
	  
          /* woj MAXNESTING */
	  int maxnesting=mod_ptr->get_count("MAXNESTING");
	  Put_Metric_Node(MAXNESTING_NODE_NAME,maxnesting);
...
void CCCC_Xml_Stream::Other_Extents() 
{
   ...
   while(extent_ptr!=NULL)
   {
      ...
      Put_Metric_Node(JAVADOC_NODE_NAME,extent_ptr->get_count("JAVADOC"),""); /* WOJ */
      Put_Metric_Node(NOCCCC_NODE_NAME,extent_ptr->get_count("NOCCCC"),""); /* WOJ */
      Put_Metric_Node(MAXNESTING_NODE_NAME,extent_ptr->get_count("MAXNESTING"),""); /* WOJ */
	  Put_Metric_Node(LINES_NODE_NAME,extent_ptr->get_count("LINES"),""); /* marcin */
...
void CCCC_Xml_Stream::Module_Detail(CCCC_Module *module_ptr)
{
  ...
  while(eIter!=module_ptr->extent_table.end())
  {
     ...
     /* WOJ */
     int javadoc=ext_ptr->get_count("JAVADOC");
     Put_Metric_Node(JAVADOC_NODE_NAME,javadoc);
     
     int nocccc=ext_ptr->get_count("NOCCCC");
     Put_Metric_Node(NOCCCC_NODE_NAME,nocccc);
     
     int maxnesting=ext_ptr->get_count("MAXNESTING");
     Put_Metric_Node(MAXNESTING_NODE_NAME,maxnesting);

	 int lines=ext_ptr->get_count("LINES");
          Put_Metric_Node(LINES_NODE_NAME,lines);
...
void CCCC_Xml_Stream::Procedural_Detail(CCCC_Module *module_ptr)
{
  ...
  CCCC_Module::member_map_t::iterator iter = module_ptr->member_map.begin();
	...
	{ 
	  ...
	  /* WOJ */
	  int javadoc=mem_ptr->get_count("JAVADOC");
	  Put_Metric_Node(JAVADOC_NODE_NAME,javadoc);
	  int visibility=mem_ptr->get_visibility();
          switch(visibility)
          {
          case vPUBLIC:
            visibility = 2;
	  ...
	  Put_Metric_Node(VISIBILITY_NODE_NAME,visibility);

          int nocccc=mem_ptr->get_count("NOCCCC");
          Put_Metric_Node(NOCCCC_NODE_NAME,nocccc);
	  
          int maxnesting=mem_ptr->get_count("MAXNESTING");
          Put_Metric_Node(MAXNESTING_NODE_NAME,maxnesting);
			
		int lines=mem_ptr->get_count("LINES");
          Put_Metric_Node(LINES_NODE_NAME,lines);
