	      ...
	      UseType ut, bool allocate_lexcounts) 
{
  ...
  if(allocate_lexcounts==true)
    {
	int nesting = 0;
	//cout << "next extent start line = " << startLine << " end line = " << endLine << endl;
	  ...
	  ++llcmIter)
	{
	  ...
	  for(i=0; i<tcLAST; i++)
	    {
			// skip summing max nesting 
		  if (i == tcMAXNESTING)
				continue;	
	      lexical_counts_for_this_extent[i]+=(*llcmIter).second[i];

		  // marcin: calculate max nesting level	
		  if (i == tcNESTING)
		  {
				nesting += (*llcmIter).second[i];
    	  } 
	    }
		//cout << "nesting = " << nesting << endl; 
		if (nesting > lexical_counts_for_this_extent[tcMAXNESTING])
		{
			lexical_counts_for_this_extent[tcMAXNESTING]=nesting;
		}
	}
	//cout << "max Nesting = " << lexical_counts_for_this_extent[tcMAXNESTING] << endl;
      ...
      lexcount_str << "LOC:" << lexical_counts_for_this_extent[tcCODELINES]
		   ...
		   << " JAVADOC:" << lexical_counts_for_this_extent[tcJAVADOC] /* WOJ */
		   << " NOCCCC:" << lexical_counts_for_this_extent[tcNOCCCC] /* WOJ */
		   << " MAXNESTING:" << lexical_counts_for_this_extent[tcMAXNESTING] /* WOJ */
    	   << " LINES:" << lexical_counts_for_this_extent[tcLINES] /* WOJ */
		   << ends;
					...
					UseType ut)
{
  ...
  if(memberName.size()>0)
  {
    ...
    // TODO: explain hack
    function_line.Insert(description);
...
//cout << moduleName << " " << memberName << " " << paramList << " " <<  description << endl;
...
void ParseStore::endOfLine(int line)
{
  ...
  if(ANTLRToken::bCodeLine)
  {
	...
	// marcin: count all lines
	pendingLexicalCounts[tcLINES]++;
}
