...
#token NOCCCC "NOCCCC" << IncrementCount(tcNOCCCC); skip(); >>
#token NOSONAR "NOSONAR" << IncrementCount(tcNOCCCC); skip(); >>
#token COMLINE_END  "\n" << 
...
//#token NOCCCC "//NOCCCC" << IncrementCount(tcNOCCCC); mode(NOCCCC); skip(); >>
//#token NOSONAR "//NOSONAR" << IncrementCount(tcNOCCCC); mode(NOCCCC); skip(); >>
//#lexclass NOCCCC
//#token CCCC_END  "\n" << 
//  endOfLine(*this);
//  mode(START); 
...
#token COMMULTI "/\*\*" << mode(JAVADOC); skip(); >>
#lexclass JAVADOC
#token COMMULTI_END "\*/" << 
...
#token LBRACE "\{" << ANTLRToken::IncrementNesting(); IncrementNesting(); >>
#token RBRACE "\}" << ANTLRToken::DecrementNesting(); DecrementNesting(); >>
#token LPAREN "\(" << ; >>
#token RPAREN "\)" << ; >>
#token LBRACK "\[" << ; >>
#token RBRACK "\]" << ; >>
...
class_item_qualifier :
    ...
    | INLINE
    | linkage_qualifier /* WOJ: fix for template before method declaration inside class */
	;	
...
indirection_modifier[string& indirMods] :
           ...
           | ASTERISK       << indirMods+="*"; >>
           | AMPERSAND      << indirMods+="&"; >>
           | KW_CONST /* WOJ fix for const in static initializers */
;
...
init_expr_item :
	  SIZEOF paren_block
	| paren_block instance_item[d1,d2] /*WOJ: fix for casts (int)method(params) */
	| paren_block
	| brace_block
	| (IDENTIFIER paren_block)?
	| cast_keyword angle_block paren_block
	| constant
	| instance_item[d1,d2] /* WOJ: fix for ampersand and asterisks in constructor initializers */
	| NEW init_expr_item /* WOJ: fix for variable1(new type(variable2)) in constructor initializer */
	;
