...
void CCCC_Project::add_member(CCCC_Item& member_data_line) 
{
  ...
  if(
     ...
     // TODO: explain hack
     member_data_line.Extract(description)
     )
    {
// TODO: comment the hack
      if(description == "declaration")
      {
        new_member_ptr->hasDeclaration = 1;
      }
      if(description == "definition")
      {
        new_member_ptr->hasDefinition = 1;
      }
/* TODO: delete this
      cerr << "XXXXXXXXXXXX " << new_member_ptr->member_name << " " << description
      << " " << new_member_ptr->hasDeclaration
      << " " << new_member_ptr->hasDefinition
      << endl;
*/
...
/* TODO: delete this
        ...
        {
      cerr << "YYYYYYYYYY " << found_member_ptr->member_name
      << " " << found_member_ptr->hasDeclaration
      << " " << found_member_ptr->hasDefinition
      << endl;
        }
*/
        ...
        if(found_member_ptr!=new_member_ptr)
        {
          int shouldChangeName = new_member_ptr->hasDefinition && found_member_ptr->hasDefinition;
          int shouldReportError = new_member_ptr->hasDeclaration && found_member_ptr->hasDeclaration;

/* TODO: delete this
		  cerr << "aaaaaaaaaaaa " << found_member_ptr->member_name
		  << " " << found_member_ptr->hasDeclaration
		  << " " << found_member_ptr->hasDefinition
		  << endl;
*/
          ...
          if(shouldChangeName)
          {
            stringstream s;
            s << counter++;
            new_member_ptr->member_name += "@@@copy" + s.str();
            found_member_ptr=member_table.find_or_insert(new_member_ptr);
          }
          else if(shouldReportError)
          {
            stringstream s;
            s << counter++;
            new_member_ptr->member_name += "@@@error_multiple_declarations" + s.str();
            found_member_ptr=member_table.find_or_insert(new_member_ptr);
            cerr << "Warning: multiple declariations" << endl;
          }

          found_member_ptr->hasDeclaration |= new_member_ptr->hasDeclaration;
          found_member_ptr->hasDefinition |= new_member_ptr->hasDefinition;
          new_member_ptr->hasDeclaration |= found_member_ptr->hasDeclaration;
          new_member_ptr->hasDefinition |= found_member_ptr->hasDefinition;
        }
