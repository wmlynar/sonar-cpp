...
CParser::definition_or_declaration( string& scope )
{
	...
	//cout << "definition_or_declaration start\n";
...
CParser::instance_declaration( string& scopeName )
{
	...
	//cout << "instance_declaration start\n";
...
(setwd7[LA(2)]&0x10) ) {
 ...
 consume();
				...
				if ( (LA(1)==INLINE) && (setwd7[LA(2)]&0x20) ) {
...
CParser::method_signature( string& scope, string& methodName, string& paramList )
{
	...
	int startLine=LT(1)->getLine();
	//cout << "method_signature start\n";
	scoped_identifier( scope,methodName );
	param_list( scope,paramList );
	opt_const_modifier();
	if ( !guessing ) {
	
	//cout << "method_signature stop " << methodName << "\n";
	}
...
(setwd9[LA(2)]&0x20) ) {
		...
		else {FAIL(2,err22,err23,&zzMissSet,&zzMissText,&zzBadTok,&zzBadText,&zzErrk); goto fail;}
	}
...
CParser::indirection_modifier( string& indirMods )
{
	...
	zzGUESS_BLOCK
	zzGUESS
	if ( !zzrv && (LA(1)==KW_CONST) && (LA(2)==ASTERISK) ) {
		{
			zzmatch(KW_CONST); labase++;
			 consume();
			zzmatch(ASTERISK); labase++;
...
CParser::scoped_identifier( string& scope, string& name )
{
	...
	zzGUESS_BLOCK
	zzGUESS
	if ( !zzrv && (LA(1)==IDENTIFIER) && (setwd11[LA(2)]&0x2) ) {
		{
...
CParser::unscoped_member_name( string& name )
{
	...
	zzTRACEIN("unscoped_member_name");
	{
		...
		if ( !zzrv && (LA(1)==IDENTIFIER) && (LA(2)==LESSTHAN) ) {
			...
			if ( !zzrv ) zzGUESS_DONE;
			if ( (LA(1)==IDENTIFIER) && (setwd11[LA(2)]&0x40) ) {
				zzmatch(IDENTIFIER); labase++;
...
(setwd12[LA(2)]&0x8) ) {
		...
		if ( !zzrv ) zzGUESS_DONE;
		zzGUESS
		if ( !zzrv && (setwd12[LA(1)]&0x10) && (LA(2)==LBRACK) ) {
			{
				new_or_delete();
			...
			if ( !zzrv ) zzGUESS_DONE;
			if ( (setwd12[LA(1)]&0x20) && (setwd12[LA(2)]&0x40) ) {
				new_or_delete();
			}
			else {
				if ( !zzrv ) zzGUESS_DONE;
				if ( (setwd12[LA(1)]&0x80) && (setwd13[LA(2)]&0x1) ) {
					type( cv,name,indir );
					if ( !guessing ) {
					opname=name+indir;
					}
...
CParser::param_list( string& scope, string& params )
{
	...
	string param_items;
	zzGUESS
	if ( !zzrv && (LA(1)==LPAREN) && (setwd13[LA(2)]&0x8) ) {
		{
			zzmatch(LPAREN); labase++;
			 consume();
		...
		if ( !zzrv ) zzGUESS_DONE;
		if ( (LA(1)==LPAREN) && (setwd13[LA(2)]&0x10) ) {
			paren_block();
			if ( !guessing ) {
...
(setwd15[LA(2)]&0x1) ) {
		...
		if ( !zzrv ) zzGUESS_DONE;
		zzGUESS
		if ( !zzrv && (LA(1)==TYPEDEF) && (setwd15[LA(2)]&0x2) ) {
			{
			...
			if ( !zzrv ) zzGUESS_DONE;
			if ( (LA(1)==TYPEDEF) && (setwd15[LA(2)]&0x4) ) {
...
CParser::class_or_method_declaration_or_definition( string& scope )
{
	...
	string dummy;
	zzGUESS
	if ( !zzrv && (setwd16[LA(1)]&0x20) && (setwd16[LA(2)]&0x40) ) {
		{
			class_key( dummy );
		}
		zzGUESS_DONE
...
CParser::class_prefix( string& modname, string& modtype )
{
	...
	if ( (setwd17[LA(1)]&0x10) && (setwd17[LA(2)]&0x20) ) {
		class_key( modtype );
		...
		if ( (setwd17[LA(1)]&0x40) && (setwd17[LA(2)]&0x80) ) {
			class_key( modtype );
		}
...
CParser::ctor_init_item_list(void)
{
	...
	zzGUESS_BLOCK
	zzGUESS
	if ( !zzrv && (setwd19[LA(1)]&0x8) && (setwd19[LA(2)]&0x10) ) {
		{
			ctor_init_item();
			zzmatch(COMMA); labase++;
			 consume();
		...
		if ( !zzrv ) zzGUESS_DONE;
		if ( (setwd19[LA(1)]&0x20) && (setwd19[LA(2)]&0x40) ) {
			ctor_init_item();
		}
...
CParser::linkage_qualifiers(void)
{
	...
	zzGUESS_BLOCK
	zzGUESS
	if ( !zzrv && (setwd20[LA(1)]&0x2) && (setwd20[LA(2)]&0x4) ) {
		{
			linkage_qualifier();
		}
		zzGUESS_DONE
...
CParser::linkage_qualifier(void)
{
 ...
 consume();
		...
		if ( !zzrv && (LA(1)==EXTERN) && (LA(2)==STRINGCONST) ) {
			...
			if ( !zzrv ) zzGUESS_DONE;
			if ( (LA(1)==EXTERN) && (setwd20[LA(2)]&0x40) ) {
				zzmatch(EXTERN); labase++;
				 consume();
			}
			else {
				...
				if ( (LA(1)==INLINE) ) {
					...
					else {FAIL(2,err55,err56,&zzMissSet,&zzMissText,&zzBadTok,&zzBadText,&zzErrk); goto fail;}
				}
			}
		}
	}
...
CParser::opt_initializer(void)
{
	...
	zzGUESS_BLOCK
	string dummy;
	if ( (LA(1)==ASSIGN_OP) && (setwd21[LA(2)]&0x40) ) {
		zzmatch(ASSIGN_OP); labase++;
		 consume();
		init_expr();
	}
		...
		zzGUESS
		if ( !zzrv && (LA(1)==LPAREN) && (LA(2)==RPAREN) ) {
			{
				zzmatch(LPAREN); labase++;
				 consume();
				zzmatch(RPAREN); labase++;
			...
			if ( !zzrv ) zzGUESS_DONE;
			if ( (LA(1)==LPAREN) && (setwd21[LA(2)]&0x80) ) {
				zzmatch(LPAREN); labase++;
				 consume();
				init_expr();
				zzmatch(RPAREN); labase++;
...
(setwd22[LA(2)]&0x10) ) {
		...
		if ( !zzrv ) zzGUESS_DONE;
		if ( (setwd22[LA(1)]&0x20) && (setwd22[LA(2)]&0x40) ) {
			init_expr_item();
		}
...
CParser::init_expr_item(void)
{
	...
	if ( (LA(1)==SIZEOF) ) {
		...
		if ( (LA(1)==LPAREN) && (setwd23[LA(2)]&0x1) ) {
			paren_block();
			instance_item( d1,d2 );
		}
		else {
			if ( (LA(1)==LPAREN) && (setwd23[LA(2)]&0x2) ) {
				paren_block();
			}
			else {
				if ( (LA(1)==LBRACE)
					...
					if ( !zzrv && (LA(1)==IDENTIFIER) && (LA(2)==LPAREN) ) {
						...
						if ( (setwd23[LA(1)]&0x4) ) {
							...
							if ( !zzrv ) zzGUESS_DONE;
							if ( (setwd23[LA(1)]&0x8) && (setwd23[LA(2)]&0x10) && !( LA(1)==IDENTIFIER && LA(2)==LESSTHAN) ) {
								constant();
							}
							else {
								if ( !zzrv ) zzGUESS_DONE;
								if ( (setwd23[LA(1)]&0x20) && (setwd23[LA(2)]&0x40) ) {
									instance_item( d1,d2 );
								}
								else {
									if ( !zzrv ) zzGUESS_DONE;
									...
									else {FAIL(2,err63,err64,&zzMissSet,&zzMissText,&zzBadTok,&zzBadText,&zzErrk); goto fail;}
								}
							}
						}
					}
...
CParser::string_literal(void)
{
	...
	if ( !zzrv && (LA(1)==STRINGCONST) && (LA(2)==STRINGCONST) ) {
		...
		if ( !zzrv ) zzGUESS_DONE;
		if ( (LA(1)==STRINGCONST) && (setwd26[LA(2)]&0x4) ) {
			zzmatch(STRINGCONST); labase++;
			 consume();
		}
		else {FAIL(2,err80,err81,&zzMissSet,&zzMissText,&zzBadTok,&zzBadText,&zzErrk); goto fail;}
	}
...
CParser::scoped(void)
{
	...
	zzGUESS_BLOCK
	zzGUESS
	if ( !zzrv && (setwd27[LA(1)]&0x40) && (setwd27[LA(2)]&0x80) ) {
		{
			keyword();
		}
		zzGUESS_DONE
		...
		if ( !zzrv ) zzGUESS_DONE;
		zzGUESS
		if ( !zzrv && (setwd28[LA(1)]&0x1) && (setwd28[LA(2)]&0x2) ) {
			{
				op();
			}
			zzGUESS_DONE
...
CParser::angle_balanced_list(void)
{
	...
	if ( (LA(1)==LESSTHAN) && (setwd29[LA(2)]&0x8)&&(LT(1)->getType() == LESSTHAN ) ) {
		...
		if ( (LA(1)==GREATERTHAN)
			...
			if ( (setwd29[LA(1)]&0x10) && (setwd29[LA(2)]&0x20) ) {
				zzsetmatch(err90); labase++;
				 consume();
				angle_balanced_list();
			}
